...
async def get_auth_domain_service() -> AuthDomainService:
    """获取认证领域服务"""
...
async def get_audit_application_service(
    ...
    audit_repository: IAuditLogRepository = Depends(get_audit_repository),
) -> AsyncGenerator[AuditApplicationService, None]:
    """获取审计应用服务"""
    yield AuditApplicationService(
        audit_domain_service=audit_domain_service,
        audit_repository=audit_repository,
    )

async def get_auth_service(
    auth_domain_service: AuthDomainService = Depends(get_auth_domain_service),
    user_domain_service: UserDomainService = Depends(get_user_domain_service),
    user_repository: IUserRepository = Depends(get_user_repository),
) -> AsyncGenerator[AuthService, None]:
    """获取认证服务"""
    yield AuthService(
        auth_domain_service=auth_domain_service,
        user_domain_service=user_domain_service,
        user_repository=user_repository,
    )
