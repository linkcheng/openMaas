...
class AuditLogger:
    ...
    @staticmethod
    def log_data_modification(user_id: str, resource_type: str, resource_id: str, action: str, changes: dict[str, Any]):
        """记录数据修改"""
        # 遮蔽敏感字段
        safe_changes = {}
        ...
        for key, value in changes.items():
            if any(sensitive in key.lower() for sensitive in sensitive_fields):
                safe_changes[key] = SecurityValidator.mask_sensitive_data(str(value))
            else:
                safe_changes[key] = value
        
        logger.info(
            f"Data Modification: {action} on {resource_type}:{resource_id}",
            extra={
                "user_id": user_id,
